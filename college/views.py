...
from college.query import select_course,get_user,get_all_notice,get_profile,create_user_admin,create_user_student
...
def admin_post_ragistration(req):
    if req.method == "POST":
        ...
        if ps == c_pass:
            result = create_user_admin(req,f_name,l_name,email,username,ps,dept)
            if result == True:
...
def student_post_ragistration(req):
    if req.method == "POST":
        ...
        if ps == c_pass:
            result = create_user_student(req,f_name,l_name,email,username,ps,dept)
            if result == True:
